...
internal class VolunteerImplementation : IVolunteer
{
    ...
    public BO.Volunteer GetVolunteerDetails(string idNumber)
    {
        ...
        {
            throw new BlDoesNotExistException($"An unexpected error occurred while retrieving the volunteer list: {ex.Message}");
        }
        catch (DalFormatException ex)
        {
            throw new BlFormatException($"An unexpected error occurred while retrieving the volunteer list: {ex.Message}");
        }
        catch (Exception ex)
        {
            throw new BlGeneralDatabaseException($"An unexpected error occurred while retrieving the volunteer list: {ex.Message}");
...
internal class VolunteerImplementation : IVolunteer
{
    ...
    public BO.Volunteer GetVolunteerDetails(string idNumber)
    {
        ...
        {
            throw new BlDoesNotExistException($"Volunteer with ID={idNumber} was not found. {ex.Message}");
        }
        catch (DalFormatException ex)
        {
    ...
    public void UpdateVolunteerDetails(string idNumber, BO.Volunteer volunteerBO)
    {
        try
        {
            ...
            DO.Volunteer? existingVolunteer = _dal.Volunteer.Read(v => v.Id.ToString() == idNumber);
