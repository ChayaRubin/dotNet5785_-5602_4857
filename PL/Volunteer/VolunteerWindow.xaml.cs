...
namespace PL
{
    public partial class VolunteerWindow : Window, INotifyPropertyChanged
    {
        ...
        public VolunteerWindow(string idNumber, bool isSelfEdit = false, bool isAdmin = false)
        {
            ...
            DataContext = this;
            this.Closing += VolunteerPage_Closing;

            try
            {
                if (string.IsNullOrEmpty(idNumber)) // new volunteer
                {
                    CurrentVolunteer = new BO.Volunteer();
                    ButtonText = "Add";
                }
                else // existing volunteer
                {
                    CurrentVolunteer = s_bl.Volunteer.GetVolunteerDetails(idNumber);
                    ButtonText = "Update";

                    // Add observer after loading volunteer
                ...
                if (firstOpenCall != null)
                {
                    ...
                    if (call?.Status != CallStatus.Expired && assignment != null)
                    {
                        CurrentCallInProgress = new CallInProgress
                        {
                            ...
                            //DistanceFromVolunteer = s_bl.Call.CallManager.CalculateDistance(CurrentVolunteer.CurrentAddress, call.Address),
                            Status = call.Status
                        };
